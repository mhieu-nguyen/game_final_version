...
export default class day1Scene extends Phaser.Scene {
    constructor(key) {
        super({
            key: key
        });
    ...
    preload() {
        ...
        this.load.tilemapTiledJSON('map1', './assets/tilemaps/forest2.json'); 
        this.load.image('tiles1', './assets/tilesets/forest.png');
        this.load.image("imageBG", "./assets/tilesets/forestBG.png");
    ...
    create() {
        ...
        var tileset = map.addTilesetImage("forest", "tiles1"); //first arg is name you gave the tileset in Tiled
        this.add.sprite(0,0,'imageBG').setOrigin(0,0).setScrollFactor(0);
        ...
        const treelayer3 = map.createDynamicLayer("Treelayer3", tileset, 0, 0)
        treelayer3.displayHeight = this.gameHeight;
        treelayer3.displayWidth = displayWidth;  
        

        const treelayer2 = map.createDynamicLayer("Treelayer2", tileset, 0, 0)
        treelayer2.displayHeight = this.gameHeight;
        treelayer2.displayWidth = displayWidth;
        

        const treelayer1 = map.createDynamicLayer("Treelayer1", tileset, 0, 0)
        treelayer1.displayHeight = this.gameHeight;
        treelayer1.displayWidth = this.mapWidth/this.sf;
        ...
        this.add
            .text(315, 553, "press RIGHT ARROW KEY to explore the forest!", {
                font: "18px monospace",
                fill: "#ffffff",
                padding: { x: 20, y: 10 },
        ...
        this.scene1chests = totalChests.scene1chests;

        let pVariables = this.cache.json.get("pVariables");
        this.velocityIncrease = pVariables.pVar1[0];
        this.sizeIncrease = pVariables.pVar1[1];
    ...
    update(time, delta) {
        ...
        if (distance>=280 || this.chestCount == 8 || this.chestCount == 16){
            this.getting_close = false;
        }
        ...
        if (this.moving){
            if (this.predatorTriggered && distance<=180){
                this.timer+=delta;
                this.warning = true;
                if (this.timer>=1000){
                    if (this.nCoins>10){
    ...
    nextScene() {
        ...
        this.cache.task1_data.environment.push(1);
        ...
        this.cache.task1_data.Key_Presses.push(this.scene1KeyPresses);
        ...
        this.registry.set('coinsTotal', this.nCoins);     
        this.player.destroy();
        ...
        console.log(this.cache.subID);     
        var filepath = `data/subject_${this.cache.subID}/${this.cache.subID}_${date.getDate()}_${month}_${date.getFullYear()}_${date.getHours()}_${date.getMinutes()}_env1_trial_${this.cache.trial_no}.csv`;
        upload(csv, filepath);
        ...
        if (this.nCoins>=1000){
            ...
            let key = `day1Scene_${this.cache.trial_no}`;
            var scene5 = new day1Scene(key);
            this.game.scene.add(key, scene5);
            this.scene.start('SummaryDay1');
        };
    }
...
var chestEncounter =  function() {
    if (!this.chestEncountered[this.chestCount]) {
        ...
        if (!this.rateWindow[this.chestCount]) {
            ...
            this.ratingPanel = new RatingPanel(this, this.panelX, 160, this.scene1chests[this.chestCount], 15);
            this.panelTime = this.time.now;
            this.rateWindow[this.chestCount] = true;                       //only pops up once per trial 
            this.scene1KeyPresses[this.chestCount] = this.registry.get('keyCount');
                ...
                if (this.coin >= 0.00) {
                    ...
                    if (this.coin == 0.00) {
                        ...
                        this.chestmessage = "[b][color=#0BDA51]CONGRATULATION[/color][/b]!\n" +
                                            `You have found additional [b][color=#FFFF00]${Number(this.scene1chests[this.chestCount]).toFixed(2)} coins[/color][/b]!`;
                        this.feedback = createDialog(this, this.chestmessage, 'center', '24px')
                                        .setPosition(400, 180)
                                        .layout()
                                        .popUp(0)
