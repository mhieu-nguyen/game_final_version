...
export default class day4Scene extends Phaser.Scene {
    constructor(key) {
        super({
            key: key
        });
    ...
    preload() {
        ...
        this.load.tilemapTiledJSON('map4', './assets/tilemaps/graveyard2.json'); 
        this.load.image('tiles4', './assets/tilesets/graveyard2.png');
        this.load.image("imageBG4", "./assets/tilesets/graveBG.png");
    ...
    create() {
        ...
        var tileset = map.addTilesetImage("graveyard", "tiles4"); //first arg is name you gave the tileset in Tiled
        this.add.sprite(0,0,'imageBG4').setOrigin(0,0).setScrollFactor(0);
        ...
        this.add
            .text(305, 553, "press RIGHT ARROW KEY to explore the graveyard!", {
                font: "18px monospace",
                fill: "#ffffff",
                padding: { x: 20, y: 10 },
        ...
        this.scene4chests = totalChests.scene4chests;

        let pVariables = this.cache.json.get("pVariables");
        this.velocityIncrease = pVariables.pVar4[0];
        this.sizeIncrease = pVariables.pVar4[1];
    ...
    update(time, delta) {
        ...
        if (distance>=280 || this.chestCount == 10 || this.chestCount == 18 || this.chestCount == 24){
        ...
        if (this.moving){
            if (this.predatorTriggered && this.moving && distance<=190){
                this.timer+=delta;
                this.warning = true;
                if (this.timer>=1000){
                    if (this.nCoins>10){
    ...
    nextScene() {
        ...
        this.cache.task1_data.environment.push(4);
        ...
        this.cache.task1_data.Key_Presses.push(this.scene4KeyPresses);
        ...
        var filepath = `data/subject_${this.cache.subID}/${this.cache.subID}_${date.getDate()}_${month}_${date.getFullYear()}_${date.getHours()}_${date.getMinutes()}_monster_AvoidanceLearning_${this.cache.trial_no}.csv`;
        upload(csv, filepath);

        if (this.nCoins>=1000){
            this.cache.task1Completed=true;
            this.scene.start('Ending_Win');
        }
        else{
            let key = `day4Scene_${this.cache.trial_no}`;
            var scene8 = new day4Scene(key);
            this.game.scene.add(key, scene8);
            this.scene.start('Ending_Lose');
        };
    }
...
var chestEncounter =  function() {
    if (!this.chestEncountered[this.chestCount]) {
        ...
        if (!this.rateWindow[this.chestCount]) {
            ...
            this.ratingPanel = new RatingPanel(this, this.panelX, 160, this.scene4chests[this.chestCount], 'No');
            this.panelTime = this.time.now;
                                  //only pops up once per trial 
            this.scene4KeyPresses[this.chestCount] = this.registry.get('keyCount');
            ...
            this.events.once('ratingcomplete', function () {
                ...
                if (this.coin >= 0 ) {
                    ...
                    if (this.coin == 0) {
                        ...
                        this.chestmessage = "[b][color=#0BDA51]CONGRATULATION[/color][/b]!\n" +
                                            `You have found additional [b][color=#FFFF00]${Number(this.scene4chests[this.chestCount]).toFixed(2)} coins[/color][/b]!`;
                        this.feedback = createDialog(this, this.chestmessage, 'center', '24px')
                                        .setPosition(400, 180)
                                        .layout()
                                        .popUp(0)
                ...
                if (this.coin == -1){
                    ...
                    this.chestCount++;
                    if (this.chestCount == 25){
                        this.time.addEvent({delay:2000, callback: this.nextScene()});
                    };   
                }
