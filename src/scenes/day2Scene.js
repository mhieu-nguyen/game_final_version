...
export default class day2Scene extends Phaser.Scene {
    constructor(key) {
        super({
            key: key
        });
    ...
    preload() {
        ...
        this.load.tilemapTiledJSON('map2', './assets/tilemaps/desert2.json'); 
        this.load.image('tiles2', './assets/tilesets/desert2.png');
        this.load.image("imageBG2", "./assets/tilesets/desertBG.png");
    ...
    create() {
        ...
        var tileset = map.addTilesetImage("desert2", "tiles2"); //first arg is name you gave the tileset in Tiled
        this.add.sprite(0,0,'imageBG2').setOrigin(0,0).setScrollFactor(0);
        ...
        this.add
            .text(310, 553, "press RIGHT ARROW KEY to explore the dessert!", {
                font: "18px monospace",
                fill: "#ffffff",
                padding: { x: 20, y: 10 },
        ...
        this.scene2chests = totalChests.scene2chests;

        let pVariables = this.cache.json.get("pVariables");
        this.velocityIncrease = pVariables.pVar2[0];
        this.sizeIncrease = pVariables.pVar2[1];
    ...
    update(time, delta) {
        ...
        if (distance>=280 || this.chestCount == 11 || this.chestCount == 21){
    ...
    nextScene() {
        ...
        this.cache.task1_data.environment.push(2);
        ...
        this.cache.task1_data.Key_Presses.push(this.scene2KeyPresses);
        ...
        var filepath = `data/subject_${this.cache.subID}/${this.cache.subID}_${date.getDate()}_${month}_${date.getFullYear()}_${date.getHours()}_${date.getMinutes()}_env2_trial_${this.cache.trial_no}.csv`;
        upload(csv, filepath);
        ...
        if (this.nCoins>=1000){
            ...
            let key = `day2Scene_${this.cache.trial_no}`;
            var scene6 = new day2Scene(key);
            this.game.scene.add(key, scene6);
            this.scene.start('SummaryDay2');
        };
    }
...
var chestEncounter =  function() {
    if (!this.chestEncountered[this.chestCount]) {
        ...
        if (!this.rateWindow[this.chestCount]) {
            ...
            this.ratingPanel = new RatingPanel(this, this.panelX, 160, this.scene2chests[this.chestCount], 20);
            this.panelTime = this.time.now;
                                //only pops up once per trial 
            this.scene2KeyPresses[this.chestCount] = this.registry.get('keyCount');
                ...
                if (this.coin >= 0 ) {
                    ...
                    if (this.coin == 0) {
                        ...
                        this.chestmessage = "[b][color=#0BDA51]CONGRATULATION[/color][/b]!\n" +
                                            `You have found additional [b][color=#FFFF00]${Number(this.scene2chests[this.chestCount]).toFixed(2)} coins[/color][/b]!`;
                        this.feedback = createDialog(this, this.chestmessage, 'center', '24px')
                                        .setPosition(400, 180)
                                        .layout()
                                        .popUp(0)
