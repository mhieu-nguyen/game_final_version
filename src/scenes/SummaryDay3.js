...
export default class SummaryPanel extends Phaser.Scene {
    ...
    create(){
        ...
        var chestsOpen = this.cache.task1_data.Chest_Open[(this.cache.trial_no-1)*4+2];
        lossRate = 0.25;
        if (chestsOpen<25){
        ...
        if (chestsOpen == 25){
            message = "You have finished exploring the snowfield!\n" +
                      `You have collected [b][color=#FFFF00]${Number(totalCoins).toFixed(2)} coins[/color][/b] in total.\n` +
                      "You can now move on to the last environment. \n\n";
        }
        else{
            message = "You have decided to stop exploring the snowfield.\n" +
                      ...
                      "you have [b]one last chance[/b] to collect the required amount.\n\n";
        }
        ...
        this.summary = this.rexUI.add.dialog({
            ...
            title: this.rexUI.add.label({
                ...
                text: this.add.text(0, 0, `After environment 3`, {fontSize: '24px'}),
                align: 'center',
                space: {
                    left: 15,
                    right: 15,
            ...
            content: this.rexUI.add.BBCodeText(0, 0, message,
...
var transition = function(scene){
    ...
    scene.scene.start('Question_env3');
}
